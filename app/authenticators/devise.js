// all credit goes to this fellow https://stackoverflow.com/questions/33268872/unpermitted-parameter-using-devise-token-auth-gem-in-rails-api
...
export default Devise.extend({
  ...
  authenticate(identification, password) {
    return new Promise((resolve, reject) => {
      let { identificationAttributeName } = getProperties(
        this,
      ...
      this.makeRequest(data, requestOptions)
        .then(response => {
          if (response.ok) {
            response.json().then(json => {
              ...
              if (this._validate(data)) {
                ...
                run(
                  null,
                  reject,
                  "Check that server response header includes data token and valid."
                );
              }
            });
          } else {
